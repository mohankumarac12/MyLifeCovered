...
function loadCookies(cookieName, formObj) {
    ...
    if (cookie.length > 10) for (var retval = JSON.parse(cookie), i = 0; i < retval.length; i++) {
        ...
        for (var key in obj) if (key.indexOf("children") == 0) {
        	...
        	} else if(sessionStorage.getItem('src') == "iknowwhatineed") {
        		formObj.elements[key].value = obj[key];
...
function validateQuote(obj) {
	...
	/*else if ($("#calc-state").val() == 'NY') {
		...
		if(obj=="howMuchDoINeed") {
			...
			if (demographicData['coverage']== '') {
				return void displayErrorText("Please select a coverage.", "#quote_form");
			}
	    ...
	    $.ajax({
	        ...
	        success: function(data, textStatus, jqXHR) {
	        	...
	        	if(errorMessage !== ""){
		        	...
		        	document.getElementById('quote_recommended').style.display="block";
...
successCallback = function(response,profile) {
	...
	populatePremiumValues(profile.appliedQuoteVO.productPremiumsCoveragesList),
...
successCallbackForTwentyFiveDollars = function(response,profile) {
	$("#quote_recommended").find(".best-premium").html('$'+numberWithCommas(profile.affordablePremium)),
	populateCoverageValues(profile.appliedQuoteVO.productPremiumsCoveragesList),
	...
	premium=profile.affordablePremium,term = $("#rec-coverage").val(),
    ...
    $("#preselected-coverage, #comparison-coverage").val(premium), $("#preselected-term, #comparison-term").val(term),$(".calculated-term").html(term + " Years"),
    //populateRecommendedTermSlider($("#rec-coverage").val()),
    populatePremiumUpdatedTermSlider($("#rec-coverage").val()),
    //$("#original-quote-originalPolicyTerm").val($("#rec-coverage").val()),
    $("#comparison-quote-selectedPolicyTerm").val($("#rec-coverage").val()),
   // $("#original-quote-originalCoverageAmnt").val(coverage),
    $("#comparison-quote-selectedPremium").val(premium);
};
